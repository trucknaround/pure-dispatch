...
// ============================================================
function ComplianceTab({ apiFetch }) {
  ...
  async function loadComplianceData() {
    setLoading(true);
    try {
      const [logsData, connectionsData, rulesData] = await Promise.allSettled([
        apiFetch("/api/admin/audit-logs"),
        apiFetch("/api/admin/loadboard-connections"),
        apiFetch("/api/admin/source-rules"),
      ]);
      if (logsData.status === "fulfilled") setAuditLogs(logsData.value.logs || []);
      if (connectionsData.status === "fulfilled") setConnections(connectionsData.value.connections || []);
      if (rulesData.status === "fulfilled") setSourceRules(rulesData.value.rules || []);
    } catch (err) {
  ...
  if (loading) return <div style={{ color: C.muted, padding: 40 }}>Loading compliance data...</div>;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      {/* Source Rules */}
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
        <div style={{ color: C.green, fontWeight: "bold", marginBottom: 16, fontSize: 14 }}>SOURCE RULES</div>
        {sourceRules.length === 0 ? (
          <div style={{ color: C.muted, fontSize: 13 }}>No source rules configured</div>
        ) : (
          sourceRules.map((rule) => (
            <div key={rule.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 14, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <div style={{ color: C.text, fontWeight: "bold", fontSize: 14 }}>{rule.source.toUpperCase()}</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <span style={{ color: rule.restricted ? C.red : C.green, border: `1px solid ${rule.restricted ? C.red : C.green}`, padding: "2px 8px", borderRadius: 4, fontSize: 10 }}>
                    {rule.restricted ? "RESTRICTED" : "UNRESTRICTED"}
                  </span>
                  <span style={{ color: rule.can_cache ? C.green : C.yellow, border: `1px solid ${rule.can_cache ? C.green : C.yellow}`, padding: "2px 8px", borderRadius: 4, fontSize: 10 }}>
                    {rule.can_cache ? "CACHEABLE" : "NO CACHE"}
                  </span>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, fontSize: 12 }}>
                <div style={{ color: rule.allow_direct_contact ? C.green : C.red }}>
                  {rule.allow_direct_contact ? "✓" : "✗"} Direct Contact
                </div>
                <div style={{ color: rule.require_redirect_for_booking ? C.yellow : C.green }}>
                  {rule.require_redirect_for_booking ? "Redirect Booking" : "✓ Direct Booking"}
                </div>
                <div style={{ color: rule.allow_message_generation ? C.green : C.red }}>
                  {rule.allow_message_generation ? "✓" : "✗"} Message Generation
                </div>
              </div>
            </div>
          ))
      ...
      {/* 123 User Connections */}
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
        <div style={{ color: C.green, fontWeight: "bold", marginBottom: 16, fontSize: 14 }}>
          123LOADBOARD USER CONNECTIONS ({connections.length})
        </div>
        {connections.length === 0 ? (
          <div style={{ color: C.muted, fontSize: 13 }}>No users have connected 123Loadboard accounts</div>
        ) : (
          connections.map((conn) => (
            <div key={conn.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 14, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div>
                <div style={{ color: C.text, fontSize: 13 }}>{conn.account_email}</div>
                <div style={{ color: C.muted, fontSize: 11 }}>User: {conn.user_id?.slice(0, 12)}...</div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ color: conn.is_active ? C.green : C.red, border: `1px solid ${conn.is_active ? C.green : C.red}`, padding: "2px 8px", borderRadius: 4, fontSize: 10 }}>
                  {conn.is_active ? "ACTIVE" : "INACTIVE"}
                </span>
                <div style={{ color: C.muted, fontSize: 11 }}>
                  Last used: {conn.last_used_at ? new Date(conn.last_used_at).toLocaleDateString() : "Never"}
                </div>
              </div>
            </div>
          ))
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20 }}>
        ...
        {auditLogs.length === 0 ? (
          <div style={{ color: C.muted, fontSize: 13 }}>No audit log entries yet</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {auditLogs.slice(0, 50).map((log) => (
              <div key={log.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.border}`, flexWrap: "wrap", gap: 6 }}>
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <span style={{
                    color: log.compliance_decision === "ALLOWED" ? C.green : log.compliance_decision?.includes("BLOCKED") ? C.red : C.yellow,
                    fontSize: 10,
                    border: `1px solid ${log.compliance_decision === "ALLOWED" ? C.green : log.compliance_decision?.includes("BLOCKED") ? C.red : C.yellow}`,
                    padding: "2px 6px",
                    borderRadius: 4,
                  }}>
                    {log.compliance_decision}
                  </span>
                  <span style={{ color: C.blue, fontSize: 12 }}>{log.source}</span>
                  <span style={{ color: C.muted, fontSize: 12 }}>{log.action}</span>
                  {log.load_id && <span style={{ color: C.dim, fontSize: 11 }}>Load: {log.load_id}</span>}
                </div>
...
export default function AdminDashboard() {
  ...
  const authToken = localStorage.getItem("supabase_token") || localStorage.getItem("authToken") || "";
      ...
      <div style={{ padding: "24px", maxWidth: 1400, margin: "0 auto" }}>
        ...
        {activeTab === "users" && (
          <div>
            ...
            <div style={{ color: C.muted, fontSize: 12, marginBottom: 16 }}>{filteredUsers.length} users shown</div>
        ...
        {activeTab === "incidents" && (
          <div>
            <div style={{ color: C.muted, fontSize: 12, marginBottom: 16 }}>{incidents.length} total incidents logged</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {incidents.length === 0 ? (
          ...
          <div style={{ maxWidth: 600 }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {broadcastStatus && broadcastStatus !== "sending" && (
                <div style={{ color: broadcastStatus.includes("Sent") ? C.green : C.red, fontSize: 14, textAlign: "center" }}>
                  {broadcastStatus}
                </div>
              )}
            </div>
        ...
        {activeTab === "compliance" && (
          <ComplianceTab apiFetch={apiFetch} />
        )}
      </div>
    </div>
  );
